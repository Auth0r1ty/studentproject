...
class Player (pygame.sprite.Sprite):
    ...
    def movePlayer(self, x, y, sprite_list):
        ...
        if self.rect.y % 50 == 0 and self.rect.x % 50 == 0:
            ...
            if currTerrain.fieldType == config.StaticEl.path:
                self.path_checked += 1
        ...
        for temp in collision_list:
            if temp.__class__.__name__ != "Player":
                ...
                if temp.__class__.__name__ == "Enemy": #ja dodao ovaj IF by DJOLE
                    self.lives -= 1
                    self.rect.x = self.pom_x
                    self.rect.y = self.pom_y
    ...
    def get_score(self):
        ...
        for i in range(0, 12):
            for j in range(0, 16):
                if (self.gameTerrain[i][j]).fieldType == config.StaticEl.pathPlayer1:
                    sum += 1
    ...
    def get_score2(self):
        ...
        for i in range(0, 12):
            for j in range(0, 16):
                if (self.gameTerrain[i][j]).fieldType == config.StaticEl.pathPlayer2:
                    sum += 1
